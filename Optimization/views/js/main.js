...
var resizePizzas = function(size) { 
  ...
  // Took the variables dx and newwidth from the loop and placed them outside the loop improve performance
  function changePizzaSizes(size) {
    var randPizzaCont = document.querySelectorAll(".randomPizzaContainer");
    var dx = determineDx(randPizzaCont[0], size);
    var newwidth = (randPizzaCont[0].offsetWidth + dx) + 'px';
    for (var i = 0, j = randPizzaCont.length; i < j; i++) {
...
//Placed the var pizzasDiv outside the loop to improve performance
...
// The code below for dealing with requestAnimationFrame was taken from "http://www.html5rocks.com/en/tutorials/speed/animations/". 

var latestKnownScrollY = 0;
var ticking = false; // Tells us if the position of the pizzas are being updated or not
...
//The function requestTick() checks if the pizza positions are updating. 
//If the positions are not updating then it requests an Animation Frame.
//If they are updating then it does not run and thereby improves performance.
...
function updatePositions() {
  ...
  for (var i = 0, j = items.length; i < j; i++) {
    ...
    items[i].style.transform = "translateX("+ 100 * phase + "px)"; // Used translateX instead of updating left property of each element as it is more efficient
  }
